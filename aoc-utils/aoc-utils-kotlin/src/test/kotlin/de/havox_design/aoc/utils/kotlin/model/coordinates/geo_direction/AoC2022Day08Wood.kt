...
class AoC2022Day08Wood {
    private var rows = emptyList<List<AoC2022Day08Tree>>().toMutableList()

    fun addRow(row: List<AoC2022Day08Tree>) {
        rows += row
    }

    fun addRow(row: String) {
        val trees = emptyList<AoC2022Day08Tree>().toMutableList()

        for (index in row.indices) {
            trees += AoC2022Day08Tree(row.substring(index, index + 1).toInt())
        }
    ...
    fun getTree(row: Int, col: Int): AoC2022Day08Tree =
        rows[row][col]
