...
class ReservoirResearch(private var filename: String) {
    ...
    fun processTask2(): Any =
        process(getResourceAsText(filename))
            ...
            .count { it is Aoc2018Day17StillWater }

    private fun process(input: List<String>): Ground {
        val veins = input
            .parseWith(DEFINITION) { (axis, a, b1, b2) ->
                when (axis) {
                    "x" -> (b1.toInt()..b2.toInt()).map { b -> Aoc2018Day17Clay(Coordinate(a.toInt(), b)) }
                    else -> (b1.toInt()..b2.toInt()).map { b -> Aoc2018Day17Clay(Coordinate(b, a.toInt())) }
                }
            }
            .flatten()
